...
SELECT
  ...
  -- Seasonality (FIXED: Remove COALESCE defaults to allow NULL)
  ...
  -- Filters (FIXED: Extract first element from JSON arrays)
  ...
  -- holiday_occasion (FIXED: Extract first element from JSON array)
  (SELECT JSON_UNQUOTE(JSON_EXTRACT(pav.value, '$[0]')) FROM product_attribute_values pav
