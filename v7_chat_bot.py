"""
V7 FLOWER CHATBOT - ARCHITECTURE OVERVIEW
==========================================
...
4. Executes queries against MySQL database (flowers_view)
5. Renders results in a user-friendly format

MIGRATED FROM V6:
- Changed from PostgreSQL to MySQL
- Updated SQL syntax (booleans, DISTINCT ON, RAND())
- Now queries flowers_view instead of flowers table
...
# MySQL database connection string (PlanetScale)
# Contains flower product data: names, prices, colors, seasonality, etc.
# Uses flowers_view which denormalizes the normalized MySQL schema
...
# connect_args with ssl config for PlanetScale SSL requirement
...
COLOR FILTERING
- AND logic: if user says "red and white": has_red = 1 AND has_white = 1 AND colors_raw IS NOT NULL
- OR logic: if user says "red or white": (has_red = 1 OR has_white = 1) AND colors_raw IS NOT NULL
- Single color (e.g., pink): has_pink = 1 AND colors_raw IS NOT NULL
...
- Always include is_year_round = 1.
...
def build_seasonality_condition(event_month: int, event_day: int) -> str:
    ...
    - It's year-round (is_year_round = 1), OR
...
def build_sql_from_memory(memory: MemoryState) -> str:
    ...
    if memory.colors:
        ...
        for color in memory.colors:
            ...
            if "cool colors" in color_lower or "cool tones" in color_lower:
                color_conditions.append("(has_blue = 1 OR has_purple = 1 OR has_green = 1)")
            elif "warm colors" in color_lower or "warm tones" in color_lower:
                color_conditions.append("(has_red = 1 OR has_orange = 1 OR has_yellow = 1)")
            elif "neutral colors" in color_lower or "neutral tones" in color_lower:
                color_conditions.append("(has_white = 1 OR has_pink = 1)")
            # Basic colors using boolean columns
            elif color_lower == "red":
                color_conditions.append("has_red = 1")
            elif color_lower == "pink":
                color_conditions.append("has_pink = 1")
            elif color_lower == "white":
                color_conditions.append("has_white = 1")
            elif color_lower == "yellow":
                color_conditions.append("has_yellow = 1")
            elif color_lower == "orange":
                color_conditions.append("has_orange = 1")
            elif color_lower == "purple":
                color_conditions.append("has_purple = 1")
            elif color_lower == "blue":
                color_conditions.append("has_blue = 1")
            elif color_lower == "green":
                color_conditions.append("has_green = 1")
            else:
    ...
    if memory.exclude_colors:
        ...
        for color in memory.exclude_colors:
            ...
            if "cool colors" in color_lower or "cool tones" in color_lower:
                exclude_color_conditions.append("(has_blue = 0 AND has_purple = 0 AND has_green = 0)")
            elif "warm colors" in color_lower or "warm tones" in color_lower:
                exclude_color_conditions.append("(has_red = 0 AND has_orange = 0 AND has_yellow = 0)")
            elif "neutral colors" in color_lower or "neutral tones" in color_lower:
                exclude_color_conditions.append("(has_white = 0 AND has_pink = 0)")
            # Basic colors using boolean columns
            elif color_lower == "red":
                exclude_color_conditions.append("has_red = 0")
            elif color_lower == "pink":
                exclude_color_conditions.append("has_pink = 0")
            elif color_lower == "white":
                exclude_color_conditions.append("has_white = 0")
            elif color_lower == "yellow":
                exclude_color_conditions.append("has_yellow = 0")
            elif color_lower == "orange":
                exclude_color_conditions.append("has_orange = 0")
            elif color_lower == "purple":
                exclude_color_conditions.append("has_purple = 0")
            elif color_lower == "blue":
                exclude_color_conditions.append("has_blue = 0")
            elif color_lower == "green":
                exclude_color_conditions.append("has_green = 0")
            else:
    ...
    sql = f"""
    WITH filtered AS (
        -- Step 1: Apply all filters and get distinct products (MySQL uses GROUP BY instead of DISTINCT ON)
        SELECT 
            unique_id, product_name, variant_name, description_clean, variant_price,
            colors_raw, diy_level, product_type_all_flowers, group_category,
            recipe_metafield, holiday_occasion, is_year_round, non_color_options,
...
def render_rows(rows: List[Dict[str, Any]]) -> str:
    ...
    out_lines = []
    # Filter to only valid dictionary rows
    valid_rows = [r for r in rows if isinstance(r, dict)]
    out_lines.append(f"Here are {min(len(valid_rows), 6)} recommendations I have:\n")
    for i, r in enumerate(valid_rows[:6], start=1):
...
def run_sql(sql: str) -> (List[Dict[str, Any]], float):
    ...
    try:
        with ENGINE.connect() as conn:
            ...
            # SQLAlchemy 2.0+ returns Row objects with ._mapping attribute
            # SQLAlchemy 1.4 returns Row objects with ._asdict() method
            # pymysql with DictCursor returns dicts directly
            rows = []
            for row in result:
                try:
                    converted_row = None
                    
                    # Method 1: SQLAlchemy 2.0+ _mapping attribute
                    if hasattr(row, '_mapping'):
                        converted_row = dict(row._mapping)
                    # Method 2: SQLAlchemy 1.4 _asdict() method
                    elif hasattr(row, '_asdict'):
                        converted_row = row._asdict()
                    # Method 3: Already a dictionary (pymysql DictCursor)
                    elif isinstance(row, dict):
                        converted_row = row
                    # Method 4: Row object - try to get column names from result
                    else:
                        ...
                        try:
                            if hasattr(result, 'keys') and callable(result.keys):
                                ...
                                keys = list(row._fields) if hasattr(row._fields, '__iter__') else []
                            ...
                            if keys and isinstance(keys, (list, tuple)):
                                ...
                                if isinstance(row, (tuple, list)):
                                    converted_row = dict(zip(keys, row))
                                else:
                                    ...
                                    for key in keys:
                                        if isinstance(key, str):
                                            try:
                                                converted_row[key] = getattr(row, key, None)
                                            except:
                                                converted_row[key] = None
                        except Exception as e:
                            # If column access fails, try __dict__
                            if hasattr(row, '__dict__'):
                                converted_row = row.__dict__
                    
                    # Validate that we got a dictionary before adding
                    if converted_row is not None and isinstance(converted_row, dict):
                        rows.append(converted_row)
                    else:
                        print(f"Warning: Could not convert row to dict: {type(row)}, converted: {type(converted_row)}, skipping")
                        continue
                        
                except Exception as e:
                    print(f"Warning: Error converting row to dict: {e}, row type: {type(row)}")
                    import traceback
                    traceback.print_exc()
                    continue
            
            # Final validation: ensure all rows are dictionaries
            validated_rows = []
            for i, r in enumerate(rows):
                if isinstance(r, dict):
                    validated_rows.append(r)
                else:
                    print(f"Warning: Row {i} is not a dict (type: {type(r)}), skipping")
            
            rows = validated_rows
            
            # Ensure we have at least empty list, not None
            if rows is None:
                rows = []
                
    except Exception as e:
        # Re-raise with more context
        # Ensure we're working with the exception message, not the exception object itself
        if isinstance(e, str):
            error_msg = e
        else:
            error_msg = str(e)
        # Don't include full SQL if it's very long, just show first part
        sql_preview = sql[:200] + "..." if len(sql) > 200 else sql
        # Create a new exception to avoid any issues with the original
